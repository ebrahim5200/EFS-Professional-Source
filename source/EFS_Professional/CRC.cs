namespace EFS_Professional
{
    ...
    public class CRC
    {
        ...
        private static ushort ComputeCRC(ushort crc, byte data) => 
            Convert.ToUInt16((ushort) (((ushort) (crc >> 8)) ^ CRCTable[(crc ^ data) & 0xff]));
        ...
        public static byte[] GetBufferWithCRC(byte[] bs) => 
            GetBufferWithCRC(bs, bs.Length);
        ...
        public static byte[] GetCRCAsByteFromByte(byte[] incomingBytes)
        {
            CDMACRC cdmacrc = new CDMACRC(CDMACRC.InitialCrcValue.NonZero1);
            return FlipByteArray(InvertBytes(cdmacrc.ComputeChecksumBytes(FlipByteArray(incomingBytes))));
        }
